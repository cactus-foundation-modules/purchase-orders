...
// One place that resolves the six permission keys, so a screen and the route
// behind it can never disagree about who is allowed to do what. Admins hold all
// six implicitly, exactly as they do everywhere else on the platform. One round
// trip, not six - every admin page in this module asks.
export async function getPoAccess(user: SessionUser): Promise<PoAccess> {
  ...
  return {
    ...
    canAccess: has('purchase-orders.access') || create || approve || receive || bills,
    canCreate: create,
    canApprove: approve,
    canReceive: receive,
    canBills: bills,
